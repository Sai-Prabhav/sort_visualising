...
int sketch_size = 1260;
int width = 1260;
int height = 1260;

void draw_bar(int height, int index, int max, char color[])
{
    int d = sketch_size / max;
    int width = d * 0.8;
    set_fill(color);
    draw_rectangle(
        (d * index) + (d / 2) - (sketch_size / 2),
        d * (height) * 0.5 - sketch_size / 2,
        width,
        d * (height));
    set_fill("#000000");
}
void draw_chart(int arr[], int len, int j)
{
    for (int i = 0; i < len; i++)
    {
        if (i != j)
        {
            draw_bar(arr[i], i, len, "#000000");
        }
        else
        {
            draw_bar(arr[i], i, len, "#ff0000");
        }
    }
}

void set_sketch_size(int size)
{
    sketch_size = size;
    width=size;
    height=size;
